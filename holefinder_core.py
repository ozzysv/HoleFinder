...
def get_config_dir():
    """Return a cross-platform user config directory for HoleFinder."""
    if sys.platform == "win32":
        base = os.environ.get("LOCALAPPDATA")
        path = Path(base) if base else (Path.home() / "AppData" / "Local")
        path = path / "HoleFinder"
    elif sys.platform == "darwin":
        path = Path.home() / "Library" / "Application Support" / "HoleFinder"
    else:
...
class HoleFinderFrame(wx.Frame):
    def __init__(self):
        ...
        try:
            icon = wx.Icon(str(PLUGIN_DIR / "holefinder_32.png"), wx.BITMAP_TYPE_PNG)
            self.SetIcon(icon)
        except Exception:
            pass
        ...
        self.github_link = wx.adv.HyperlinkCtrl(
            panel,
            label="github.com/ozzysv/HoleFinder",
            url="https://github.com/ozzysv/HoleFinder",
        )
    ...
    def on_update(self, evt):
        try:
            ...
            wx.MessageBox(str(e), "HoleFinder", wx.OK | wx.ICON_ERROR)
    ...
    def on_locate(self, evt):
        ...
        try:
            ...
            wx.MessageBox(f"Locate failed:\n{e}", "HoleFinder", wx.OK | wx.ICON_ERROR)
...
def run():
    ...
    checker = wx.SingleInstanceChecker(f"HoleFinder-{wx.GetUserId()}")

    if checker.IsAnotherRunning():
        wx.MessageBox(
            "HoleFinder is already running.",
            "HoleFinder",
